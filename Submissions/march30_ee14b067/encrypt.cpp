...
#include<math.h>
#include<string.h>
unsigned long long int expansionBox(unsigned long long int a)
{
...
unsigned long long int KeyGeneration(unsigned long long int Key,int a) //Didn't implement this
{
	return 1;
}
...
int main(int argc, char **argv)
{
	...
	FILE *fp1;
	fp1= fopen("enc_alice.txt","w");
	int i=0;
	char str1[2];
	char str2[2];
	unsigned long int ascii1=0,ascii2=0,ascii3=0,ascii4=0;
	...
	{
		char ch2 = fgetc(fp);
		ascii1= (int)ch2 + 256*((int)ch1); //Converting the 8 bit characters to a single 16 bit integer block for easy calculations
		char ch3 = fgetc(fp);
		char ch4 = fgetc(fp);
		ascii2= (int)ch4 + 256*((int)ch3);
		for(i=0;i<rounds;i++) //Iteration over the given number of rounds.
		{
			if(i<rounds-1)
			{
				ascii3= ascii2;
				expa= expansionBox(ascii2); //ExpansionBox Already Written
				expa= expa^Keys;  // Key Generation Algorithm
				ascii4= SBox(expa); //SBox already written
				ascii4= Diffusion(ascii4); //Diffusion also already written
				ascii4= ascii4^ascii1;
				ascii1= ascii3;
				ascii2= ascii4;
			}
			else
		...
		fflush(NULL);
		fprintf(fp1,"%ld",ascii1);
		fputc(32,fp1);
		fprintf(fp1,"%ld",ascii2);
		fputc(32,fp1);
	}
	return 0;
}
